...
// import { useFavContext } from "../contexts/favourite/FavProvider.jsx";
...
//     const { addToFavorites, removeFromFavorites, isFavorite: checkFavorite } = useFavContext();
//     const { user } = useAuth()
...
//         if (!user) return; // ако не е логнат
//         try {
//             if (checkFavorite(product.id)) {
//                 await removeFromFavorites(product.id);
//             } else {
//                 await addToFavorites(user.userId, product.id);
//             }
//         } catch (err) {
...
//                             checkFavorite(product.id) && styles.favoriteActive
//                         ]}
//                         onPress={toggleFavorite}
//                     >
//                         <Text style={styles.favoriteIcon}>
//                             {checkFavorite(product.id) ? "❤️" : "🤍"}
//                         </Text>
//                     </TouchableOpacity>
//                 </View>
...
export default function ProductScreen({ route, navigation }) {
    const { productId } = route.params;
    const { addToFavorites, removeFromFavorites, isFavorite } = useFavContext();
    const { user } = useAuth();
    const { addToCart } = useCart();
    ...
    useEffect(() => {
        async function fetchProduct() {
            try {
                const result = await shopService.fetchGetProductsById(productId);
    ...
    const toggleFavorite = async () => {
        ...
        if (isFavorite(product.id)) {
            await removeFromFavorites(product.id); // използвай id от базата, ако е различно
        } else {
            ...
            <View style={styles.infoCard}>
                <View style={styles.titleRow}>
                    ...
                    <TouchableOpacity
                        style={[
                            styles.favoriteButton,
                            isFavorite(product.id) && styles.favoriteActive
                        ]}
                        onPress={toggleFavorite}
                    >
                        <Text style={styles.favoriteIcon}>
                ...
                <View style={styles.priceSection}>
                    {product.isPromoPrice ? (
                        <>
                            <Text style={styles.promoPrice}>{product.isPromoPrice} €</Text>
